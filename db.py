...
def get_schedule_lifecycle(year: int, month: int) -> dict:
    ...
    return {
        ...
        "state":"legacy_published" if state.get("has_published") else "draft",
...
def finalize_schedule_v2591(year: int, month: int, final_json: dict) -> dict:
    rows=_data(_retry_db(lambda: client().rpc("finalize_schedule_v2591",{
