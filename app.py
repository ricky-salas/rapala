...
APP_VERSION = "2.5.91 COLORED FINALIZATION + CONTROLLED SWAP WINDOW + FINAL COHORT EMAIL"
...
def backup_table(y,m,result,backup_rows_override=None):
    slots={s.idx:s for s in make_slots(y,m)}
    rows=[]
    backup_rows=list(backup_rows_override) if backup_rows_override is not None else db.list_backups(y,m)
...
def build_xlsx(y,m,result,document_status=None,backup_rows_override=None):
    ...
    detail_start=rr+2; bdf=backup_table(y,m,result,backup_rows_override=backup_rows_override)
    if not bdf.empty:
...
def swap_window_open_emails(y,m,result,deadline,window_days):
    ...
    for pp in DEFAULT_PEOPLE:
        i=pp["initials"]; email=str((settings.get(i,{}) or {}).get("email") or "").strip(); send_date=date.today().isoformat(); kind=f"swap_window_open_{y}_{m:02d}"
        if not email:
            detail=tr("missing_email")
            try: db.record_email(i,kind,y,m,send_date,"failed",detail)
            except Exception: pass
            results.append((i,"failed",detail)); continue
        subject=(f"{month_label(y,m)} grafikas patvirtintas — apsikeitimai iki {deadline_text}" if lang=="LT" else f"{month_label(y,m)} schedule confirmed — swaps until {deadline_text}")
        body=(
            f"Sveiki,\n\nR.S. patvirtino {month_label(y,m)} SYSTEM grafiką. Tai yra užšaldyta pradinė versija prieš individualius apsikeitimus.\n\n"
            f"Turite {int(window_days)} d. pateikti naują individualaus apsikeitimo prašymą. Terminas: {deadline_text} (Lietuvos laiku).\n"
            f"Po termino naujų apsikeitimų kurti nebus galima, nebent R.S. individualiai suteiks pavėluoto apsikeitimo leidimą. Jau pateikti prašymai gali būti užbaigti, o R.S. atliks galutinę jų priežiūrą.\n\n"
            f"Po R.S. galutinio patvirtinimo dabartinis ACTUAL grafikas taps FINAL versija, kuri bus pateikta administracijai.\n"
            if lang=="LT" else
            f"Hello,\n\nR.S. confirmed the {month_label(y,m)} SYSTEM schedule. This is the frozen baseline before individual swaps.\n\n"
            f"You have {int(window_days)} day(s) to create a new individual swap request. Deadline: {deadline_text} (Lithuania time).\n"
            f"After the deadline, new swaps are closed unless R.S. grants you individual late-swap access. Existing requests may still be completed and R.S. performs final oversight.\n\n"
            f"After R.S. final confirmation, the current ACTUAL schedule becomes the FINAL version submitted to administration.\n"
        )
        if public: body += (f"\nPortalas: {public}\n" if lang=="LT" else f"\nPortal: {public}\n")
        ok,detail=send_email(email,subject,body,build_ics(y,m,result,i),f"{safe_filename(i)}_{y}_{m:02d}.ics")
...
def late_swap_access_email(y,m,grant):
    ...
    subject=(f"Pavėluoto apsikeitimo leidimas — iki {exp_text}" if lang=="LT" else f"Late swap access granted — until {exp_text}")
    body=(f"Sveiki,\n\nR.S. individualiai atidarė jums pavėluoto apsikeitimo prieigą {month_label(y,m)} grafikui.\nGalioja iki: {exp_text}.\nGalite sukurti iki {remaining} naujo(-ų) apsikeitimo prašymo(-ų).\nPrisijunkite į Shift Happens → Apsikeitimai.\n" if lang=="LT" else f"Hello,\n\nR.S. granted you individual late-swap access for the {month_label(y,m)} schedule.\nValid until: {exp_text}.\nYou may create up to {remaining} new swap request(s).\nOpen Shift Happens → Swaps.\n")
    public=config_value("SCHEDULER_PUBLIC_URL","").strip()
    if public: body += (f"\nPortalas: {public}\n" if lang=="LT" else f"\nPortal: {public}\n")
    ok,detail=send_email(email,subject,body)
...
def final_schedule_emails(y,m,result):
    ...
    for pp in DEFAULT_PEOPLE:
        ...
        subject=(
            f"FINAL — {month_label(y,m)} grafikas paskelbtas. Crack on, radistai!"
            if lang=="LT" else
            f"FINAL — {month_label(y,m)} schedule is live. Crack on, radiologists!"
        )
        body=(
            f"Sveiki,\n\n"
            f"Galutinis {month_label(y,m)} tvarkaraštis paskelbtas ir pateiktas administracijai. "
            f"R.S. užbaigė apsikeitimų priežiūrą ir patvirtino FINAL versiją.\n\n"
            f"Tai jau tikras FINAL — nauji įprasti ar pavėluoti apsikeitimai uždaryti. "
            f"Prisegtas jūsų galutinis .ics grafikas.\n\n"
            f"Viskas. Crack on, radistai — gero mėnesio ir kuo mažiau netikėtų „gal gali mane pakeist?“ žinučių.\n"
            if lang=="LT" else
            f"Hello,\n\n"
            f"The final {month_label(y,m)} schedule has been published and submitted to administration. "
            f"R.S. completed swap oversight and confirmed the FINAL version.\n\n"
            f"This is the real FINAL — new ordinary and late swaps are closed. "
            f"Your final .ics schedule is attached.\n\n"
            f"That’s it. Crack on, radiologists — have a good month and may the last-minute swap messages be few.\n"
        )
        if public: body += (f"\nPortalas: {public}\n" if lang=="LT" else f"\nPortal: {public}\n")
        ok,detail=send_email(email,subject,body,build_ics(y,m,result,i),f"FINAL_{safe_filename(i)}_{y}_{m:02d}.ics")
        try: db.record_email(i,kind,y,m,send_date,"sent" if ok else "failed",detail)
        except Exception: pass
        results.append((i,"sent" if ok else "failed",detail))
    return results
...
if senior_mode:
    with tabs[pos]:
        ...
        generation_locked=str(lifecycle_generation.get("state") or "") in ("swap_open","swap_closed","final")
        ...
        with c2:
            st.info(
                ...
                "Taip visas mėnesio lifecycle valdomas vienoje R.S. vietoje."
                if lang=="LT" else
                "SYSTEM confirmation and opening the swap window moved to Schedule → Finalization. "
                "This keeps the whole monthly lifecycle in one R.S. control center."
            )
...
with tabs[pos]:
    ...
    if is_seniune_account:
        st.markdown("## GRAFIKO TVIRTINIMAS" if lang=="LT" else "## SCHEDULE FINALIZATION")
        ...
        blockers=db.finalization_blockers_v2591(year,month) if state in ("swap_open","swap_closed","final") else {"pending_normal":0,"waiting_senior_apply":0,"pending_backup":0,"active_late_grants":0,"clear":False}

        if state in ("draft","legacy_published"):
            _workflow_card("1/4 — JUODRAŠTIS / SYSTEM DAR NEATIDARYTAS" if lang=="LT" else "1/4 — DRAFT / SYSTEM NOT OPENED", "R.S. pirmiausia patvirtina SYSTEM bazę ir atidaro ribotą apsikeitimų langą. SYSTEM po to lieka užšaldytas tyrimui; swapai keičia tik ACTUAL." if lang=="LT" else "R.S. first confirms the SYSTEM baseline and opens a limited swap window. SYSTEM then stays frozen for research; swaps change ACTUAL only.","draft")
            smtp_ok=smtp_ready(); missing_mail=_resident_email_preflight()
            if not smtp_ok: st.error("REAL EMAIL BLOKAS: SMTP nesukonfigūruotas. Reikia SCHEDULER_SMTP_HOST + SCHEDULER_EMAIL_FROM (ir, jei reikia, USER/PASSWORD)." if lang=="LT" else "REAL EMAIL BLOCK: SMTP is not configured. SCHEDULER_SMTP_HOST + SCHEDULER_EMAIL_FROM are required.")
            if missing_mail: st.error(("Trūksta realaus email: " if lang=="LT" else "Missing real email: ")+", ".join(missing_mail))
            a,b=st.columns(2)
            with a: swap_days=int(st.number_input("Apsikeitimų langas — dienų" if lang=="LT" else "Swap window — days",min_value=1,max_value=14,value=3,step=1,key=f"swap_days_{year}_{month}"))
            with b: end_time=st.time_input("Termino laikas" if lang=="LT" else "Deadline time",value=time(20,0),key=f"swap_end_time_{year}_{month}")
            end_date=now_lt.date()+timedelta(days=swap_days)
            proposed_deadline=datetime.combine(end_date,end_time,tzinfo=ZoneInfo("Europe/Vilnius"))
            st.caption((f"Realus terminas: {proposed_deadline.strftime('%Y-%m-%d %H:%M')} Lietuvos laiku." if lang=="LT" else f"Exact deadline: {proposed_deadline.strftime('%Y-%m-%d %H:%M')} Lithuania time."))
            can_open=bool((payload or draft_payload) and smtp_ok and not missing_mail)
            if st.button("PATVIRTINTI SYSTEM IR ATIDARYTI APSIKEITIMUS" if lang=="LT" else "CONFIRM SYSTEM AND OPEN SWAPS",type="primary",use_container_width=True,disabled=not can_open,key=f"open_swaps_{year}_{month}"):
                try:
                    if not payload:
                    ...
                    db.open_swap_window_v2591(year,month,proposed_deadline.astimezone(timezone.utc).isoformat())
                    mailres=swap_window_open_emails(year,month,result_open,proposed_deadline,swap_days)
                    failed=[x for x in mailres if x[1]!="sent"]
                    st.session_state["_finalization_flash"]=("warning" if failed else "success", (f"SYSTEM patvirtintas, apsikeitimai atidaryti. Email: {len(mailres)-len(failed)}/{len(mailres)} išsiųsta." if lang=="LT" else f"SYSTEM confirmed, swaps opened. Email: {len(mailres)-len(failed)}/{len(mailres)} sent."))
                    st.rerun()
                except Exception as exc: st.error(str(exc))
        elif state=="swap_open":
            title="2/4 — APSIKEITIMŲ LANGAS ATIDARYTAS" if not expired else "2/4 — TERMINAS PASIBAIGĖ"
            body=(f"Iki {deadline_lt.strftime('%Y-%m-%d %H:%M')} Lietuvos laiku. Nauji requestai {'BLOKUOJAMI nuo termino' if expired else 'leidžiami'}." if lang=="LT" else f"Until {deadline_lt.strftime('%Y-%m-%d %H:%M')} Lithuania time. New requests are {'blocked after the deadline' if expired else 'allowed'}." )
            _workflow_card(title if lang=="LT" else ("2/4 — SWAP WINDOW EXPIRED" if expired else "2/4 — SWAP WINDOW OPEN"),body,"expired" if expired else "swap_open")
            m1,m2,m3=st.columns(3); m1.metric("Pending normal",blockers.get("pending_normal",0)); m2.metric("Waiting R.S. apply",blockers.get("waiting_senior_apply",0)); m3.metric("Pending backup",blockers.get("pending_backup",0))
            if st.button("UŽDARYTI APSIKEITIMŲ LANGĄ" if lang=="LT" else "CLOSE SWAP WINDOW",type="primary",use_container_width=True,key=f"close_swaps_{year}_{month}"):
                try: db.close_swap_window_v2591(year,month); st.rerun()
                except Exception as exc: st.error(str(exc))
            if smtp_ready() and st.button("PAKARTOTI APSIKEITIMŲ LANGO EMAIL" if lang=="LT" else "RESEND SWAP-WINDOW EMAILS",use_container_width=True,key=f"resend_swap_mail_{year}_{month}"):
                rr=refresh_result_payload(db.load_schedule(year,month,"current"),year,month); days=max(1,(deadline_lt.date()-now_lt.date()).days) if deadline_lt else 1; mailres=swap_window_open_emails(year,month,rr,deadline_lt or now_lt,days); st.dataframe(localized_delivery_rows(mailres),use_container_width=True,hide_index=True)
        elif state=="swap_closed":
            _workflow_card("3/4 — APSIKEITIMAI UŽDARYTI / R.S. PRIEŽIŪRA" if lang=="LT" else "3/4 — SWAPS CLOSED / R.S. OVERSIGHT", "Naujų requestų kurti negalima. R.S. užbaigia laukiančius apsikeitimus; pavėlavusiam žmogui galima individualiai atidaryti ribotą prieigą." if lang=="LT" else "No new requests may be created. R.S. finishes pending oversight; an individual late-access exception may be granted.","swap_closed")
            m1,m2,m3,m4=st.columns(4); m1.metric("Pending normal",blockers.get("pending_normal",0)); m2.metric("Waiting R.S. apply",blockers.get("waiting_senior_apply",0)); m3.metric("Pending backup",blockers.get("pending_backup",0)); m4.metric("Active late access",blockers.get("active_late_grants",0))
            st.markdown("### Pavėluoto apsikeitimo leidimas" if lang=="LT" else "### Late-swap access")
            ...
            with l3: late_limit=int(st.number_input("Naujų requestų limitas" if lang=="LT" else "New-request limit",min_value=1,max_value=5,value=1,step=1,key=f"late_limit_{year}_{month}"))
            ...
            if st.button("SUTEIKTI INDIVIDUALŲ PAVĖLUOTĄ LEIDIMĄ" if lang=="LT" else "GRANT INDIVIDUAL LATE ACCESS",use_container_width=True,disabled=(not smtp_ready() or not late_email),key=f"grant_late_{year}_{month}"):
                try:
                    exp=_vilnius_now()+timedelta(hours=late_hours); grant=db.grant_late_swap_access_v2591(year,month,late_ini,exp.astimezone(timezone.utc).isoformat(),late_limit,late_reason); ok,detail=late_swap_access_email(year,month,grant); st.success(("Leidimas suteiktas ir email išsiųstas." if ok else f"Leidimas suteiktas, bet email nepavyko: {detail}")); st.rerun()
                except Exception as exc: st.error(str(exc))
            grants=db.list_late_swap_access_v2591(year,month)
            active_grants=[]
            for gr in grants:
                exp=_parse_iso_dt(gr.get("expires_at"));
                if not gr.get("revoked_at") and exp and exp>datetime.now(timezone.utc) and int(gr.get("requests_used",0))<int(gr.get("max_requests",0)): active_grants.append(gr)
            for gr in active_grants:
                exp=_parse_iso_dt(gr.get("expires_at")); exp_txt=exp.astimezone(ZoneInfo("Europe/Vilnius")).strftime("%Y-%m-%d %H:%M") if exp else "—"
                with st.container(border=True):
                    st.markdown(f"**{gr.get('initials')}** · iki {exp_txt} · {gr.get('requests_used',0)}/{gr.get('max_requests',1)} panaudota")
                    if gr.get("reason"): st.caption(str(gr.get("reason")))
                    if st.button("ATŠAUKTI LEIDIMĄ" if lang=="LT" else "REVOKE ACCESS",key=f"revoke_late_{gr.get('id')}"):
                        try: db.revoke_late_swap_access_v2591(int(gr["id"])); st.rerun()
                        except Exception as exc: st.error(str(exc))
            payload_now=db.load_schedule(year,month,"current")
            if payload_now:
                rr=refresh_result_payload(payload_now,year,month); hard=int((rr.stats or {}).get("global",{}).get("hard_errors",999))
                blockers=db.finalization_blockers_v2591(year,month); clear=bool(blockers.get("clear")) and hard==0
                st.markdown("### Galutinis patvirtinimas" if lang=="LT" else "### Final confirmation")
                if not clear:
                    st.error((f"FINAL dar blokuotas: HARD={hard}; pending normal={blockers.get('pending_normal',0)}; waiting R.S. apply={blockers.get('waiting_senior_apply',0)}; pending backup={blockers.get('pending_backup',0)}; active late={blockers.get('active_late_grants',0)}." if lang=="LT" else f"FINAL is still blocked: HARD={hard}; pending normal={blockers.get('pending_normal',0)}; waiting R.S. apply={blockers.get('waiting_senior_apply',0)}; pending backup={blockers.get('pending_backup',0)}; active late={blockers.get('active_late_grants',0)}."))
                confirm=st.checkbox("Patvirtinu, kad šis ACTUAL grafikas yra galutinė administracijai teikiama versija." if lang=="LT" else "I confirm this ACTUAL schedule is the final version for administration.",key=f"final_confirm_{year}_{month}")
                if st.button("PATVIRTINTI GALUTINĮ GRAFIKĄ ADMINISTRACIJAI" if lang=="LT" else "CONFIRM FINAL SCHEDULE FOR ADMINISTRATION",type="primary",use_container_width=True,disabled=(not clear or not confirm),key=f"make_final_{year}_{month}"):
                    try:
                        db.finalize_schedule_v2591(year,month,payload_now); mails=final_schedule_emails(year,month,rr); st.session_state["_finalization_flash"]=("success",("FINAL grafikas užrakintas ir paruoštas administracijai." if lang=="LT" else "FINAL schedule locked and ready for administration.")); st.rerun()
                    except Exception as exc: st.error(str(exc))
        elif state=="final":
            _workflow_card("4/4 — FINAL — ADMINISTRACIJAI" if lang=="LT" else "4/4 — FINAL — FOR ADMINISTRATION", "Šis snapshotas užrakintas. Nauji įprasti ir pavėluoti swapai negalimi. Po FINAL atsiradę force-majeure pakeitimai lieka atskirame ACTUAL/Emergency audite ir nekeičia administracijai užrakinto FINAL snapshot." if lang=="LT" else "This snapshot is locked. New ordinary and late swaps are closed. Post-FINAL force-majeure changes remain in ACTUAL/Emergency audit and do not rewrite the administration-locked FINAL snapshot.","final")
            fp=lifecycle.get("final_json")
            if fp:
                fr=refresh_result_payload(fp,year,month,use_actual_backups=True)
                st.download_button("ATSISIŲSTI FINAL EXCEL ADMINISTRACIJAI" if lang=="LT" else "DOWNLOAD FINAL EXCEL FOR ADMINISTRATION",build_xlsx(year,month,fr,document_status="FINAL — ADMINISTRACIJAI" if lang=="LT" else "FINAL — FOR ADMINISTRATION",backup_rows_override=lifecycle.get("final_backups") or []),file_name=f"FINAL_grafikas_{year}_{month:02d}.xlsx",mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",type="primary",use_container_width=True,key=f"final_xlsx_{year}_{month}")
                if smtp_ready() and st.button("PAKARTOTI FINAL EMAIL VISIEMS" if lang=="LT" else "RESEND FINAL EMAIL TO ALL",use_container_width=True,key=f"resend_final_mail_{year}_{month}"):
                    mailres=final_schedule_emails(year,month,fr); st.dataframe(localized_delivery_rows(mailres),use_container_width=True,hide_index=True)
    ...
    if not payload:
        if draft_payload and not is_seniune_account: st.info(tr("not_published"))
        ...
        result=refresh_result_payload(display_payload,year,month,use_actual_backups=(state!="final")); st.markdown(f"### {tr('colors')}"); st.markdown("".join(badge(p["initials"],False) for p in DEFAULT_PEOPLE),unsafe_allow_html=True); st.dataframe(style_schedule(schedule_grid(year,month,result)),use_container_width=True,height=720)
        ...
        if state!="final":
            st.caption("Tai dabartinis ACTUAL grafikas. Administracijai skirtas FINAL Excel atsiras tik po R.S. galutinio patvirtinimo." if lang=="LT" else "This is the current ACTUAL schedule. The administration FINAL Excel appears only after R.S. final confirmation.")
            c1,c2=st.columns(2)
...
with tabs[pos]:
    ...
    if currentp:
        ...
        if swap_create_allowed and src=="window":
            ...
            _workflow_card("INDIVIDUALUS PAVĖLUOTAS LEIDIMAS" if lang=="LT" else "INDIVIDUAL LATE ACCESS", (f"R.S. atidarė prieigą iki {txt}; liko {swap_perm.get('remaining','—')} naujų requestų." if lang=="LT" else f"R.S. granted access until {txt}; {swap_perm.get('remaining','—')} new request(s) remain."),"swap_closed")
        else:
            state_name="FINAL" if src=="final" else ("TERMINAS PASIBAIGĖ" if src=="expired" else "NAUJI APSIKEITIMAI UŽDARYTI")
            _workflow_card(state_name if lang=="LT" else ("FINAL" if src=="final" else "NEW SWAPS CLOSED"), ("Naujų apsikeitimo prašymų kurti negalite. Jau egzistuojančius prašymus dar galima priimti / atmesti, kol R.S. užbaigs priežiūrą." if src!="final" and lang=="LT" else "Nauji apsikeitimai uždaryti." if lang=="LT" else "No new swap requests can be created. Existing requests may still be responded to until R.S. completes oversight." if src!="final" else "The schedule is FINAL; new swaps are closed."),"final" if src=="final" else "expired")
    if not currentp: st.info(tr("not_published"))
            ...
            if st.button(tr("request_swap"),type="primary",disabled=(not preview_ok or not my_ack or not swap_create_allowed)):
                ...
                try:
                    ...
                    if "SWAP_WINDOW_CLOSED" in str(exc):
                        st.warning("Naujų apsikeitimų langas uždarytas. Jei pavėlavote, kreipkitės į R.S. dėl individualaus leidimo." if lang=="LT" else "New swaps are closed. If you are late, ask R.S. for individual late access.")
