...
def main(argv):
    ...
    try:
        ...
        # V2.5.128: inject the private SP + ŠR generation-only layer from the
        # ephemeral worker payload. It is deliberately absent from the frozen/public snapshot.
        private_by_owner=dict(payload.get("operator_private_pair_preferences") or {})
        for person in people:
            person.privileged_pair_preferences=[dict(x) for x in (private_by_owner.get(person.initials) or [])]
