# V2.5.113 — INITIALS CORRECTION + SP LEADER

## Corrected canonical initials

The roster identifiers are now exactly:
...
- PV — Pileckienė Aistė
- SŠ — Stašinskas Kipras
- GD — Giedrimas Deivydas
- ŠR — Šalaševičius Rapolas
- MŽ — Mažonavičius Ignas
- SP — Steponavičiūtė Rosita
- MG — Maleckaitė Gabrielė
- GE — Gertas Ernestas

Corrections from V2.5.112: `SK → SŠ` and `SR → SP`.

SP remains the operational senior scheduler with target adjustment -2. Dream Team is now `SP + ŠR + GE`. WESTON generation-click accounting and final swap approval authority are bound to SP. Existing linked accounts, preferences, settings, audit rows and schedule JSON are migrated in place rather than recreated.

## V2.5.112 behavior retained
...
4. Dream Team rule — SP + ŠR + GE together on CENTRO RO at least once per represented workweek when mathematically feasible.
...
ACTUAL voluntary swaps may later disturb SYSTEM fairness, but only after both residents consent and SP gives final approval. Safety/operational blockers remain enforced.
...
## Dream Team
Canonical database initials are `SP`, `ŠR`, and `GE`.
...
`Resident A proposes → Resident B accepts → WAITING FOR SP → SP APPROVE / DECLINE → ACTUAL changes`

SP sees the full pending queue. Resident acceptance alone never applies the swap.

## WESTON ledger
For SP only:
- every Generate / Regenerate button press records `+1 WESTON beer`
- displayed next to the Generate button
- persistent lifetime total and selected-month total appear in SP personal statistics
...
It provides the WESTON ledger plus exact-SP final approval fields/functions for voluntary swaps.