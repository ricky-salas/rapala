...
APP_VERSION = "2.5.172 RESIDENT SIMPLE PROFILE"
EXPECTED_ENGINE_API_VERSION = "2.5.172"
COMPATIBLE_ENGINE_API_VERSIONS = {"2.5.172"}
    ...
    with tabs[pos]:
        ...
        with c1:
            _generate_button_label=(
                ...
                if state.get("has_draft") else
                ("GENERUOTI JUODRAŠTĮ" if lang=="LT" else "GENERATE DRAFT")
            )
        ...
        if draftp:
            ...
            if not _display_health.get("publishable"):
                render_invalid_draft_guard(_display_health,compact=True)
            else:
                st.success(
                    "Juodraštis sukurtas. Eikite į Grafikas → Grafiko tvirtinimas patvirtinti grafiką arba spauskite GENERUOTI IŠ NAUJO, jei norite kito varianto."
                    if lang=="LT" else
                    "Draft created. Go to Schedule → Confirmation to approve it, or press GENERATE AGAIN if you want another version."
                )
...
with tabs[pos]:
    ...
    if lang=="LT":
        st.success(
            ...
            "Generatorius taiko ~40 val./7 d. tikslą, >48 val. workload perspėjimą, aktyvų HARD limitą iki 60 val./7 d., ≤6 darbo dienas/7 d. ir recovery po dvigubų. "
        ...
        st.dataframe(pd.DataFrame([
            {"Rangas":"1. TRUE ABSOLUTE HARD","Kas įeina":"Sauga, patvirtinta liga/atostogos, fizinis neįmanomumas, coverage; generatoriui aktyvus rolling-7 HARD limitas (iki 60 val.) ir bent 1 laisva diena/7 d.","Kaip sprendžiama":"Generuojant 100%. ~40 val. yra tikslas, >48 val. yra perspėjimas, o aktyvus rolling-7 HARD limitas (iki 60 val.) neapeinamas; kiti HARD lieka"},
        ...
        st.dataframe(pd.DataFrame([
            ...
            {"Taisyklė":"GENERATORIUS — aktyvus HARD limitas iki 60 val./7 d.","Veikimas":"Sistema taiko aktyvų Rule Profile rolling-7 HARD limitą; ~40 val. yra tikslas, >48 val. aiškiai pažymima kaip didelis krūvis.","Statusas":"GENERATION HARD"},
        ...
        st.success(
            ...
            "Generation targets ~40h/7d, flags >48h as high workload, enforces the active HARD cap up to 60h/7d, ≤6 workdays/7d and post-double recovery. "
        ...
        st.dataframe(pd.DataFrame([
            {"Rank":"1. TRUE ABSOLUTE HARD","Includes":"Safety/rest, approved absence, physical impossibility, coverage; generation uses the active rolling-7 HARD cap (up to 60h) and >=1 free day/7d","Method":"100% during generation. Post-publication voluntary swaps use consequence + ACK warnings, but ABSOLUTE/operational and labour-time blockers remain hard"},
    ...
    if lang=="LT":
        workflow_rows=[
            ...
            {"Etapas":"1. TRUE ABSOLUTE HARD","Sistema":"Randa tik saugų/fiziškai įmanomą grafiką; generuojant taiko aktyvų rolling-7 HARD limitą (iki 60h), >48h žymi kaip didelį krūvį ir saugo Onko recovery.","Vertina":"Poilsį, valandas, patvirtintą neatvykimą, coverage/overlap, tikslų mėnesio krūvį ir lygines Onko poras.","Principas":"Tikslus mėnesio targetas ir Onko 0/2/4/... yra HARD SYSTEM ir ACTUAL. Consecutive Onko gali būti tik savanoriško swapo ACK pasekmė; parity niekada neapeinama."},
            ...
            {"Etapas":"5. WEEKLY LOAD + RECOVERY","Sistema":"Water-fill'ina savaitinį valandų krūvį ir užrakina recovery frontier.","Vertina":"Rolling-7 valandas, kalendorinių savaičių spreadą, consecutive 12h doubles.","Principas":"~40 val./7 d. tikslas; >48 val. perspėjimas/avoidance; aktyvus HARD limitas iki 60 val.; po 2 doubles kita diena tik PM arba laisva, laisva preferinama."},
        ...
        workflow_rows=[
            ...
            {"Stage":"5. WEEKLY LOAD + RECOVERY","System":"Water-fills weekly hours and locks the recovery frontier.","Evaluates":"Rolling-7 hours, calendar-week spread, consecutive 12h doubles.","Principle":"Aim ~40h/7d; >48h warning/avoidance; active HARD cap up to 60h; after 2 doubles next day PM-only or off, preferring off."},
