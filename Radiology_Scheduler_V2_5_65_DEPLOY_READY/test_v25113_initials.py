...
for sl,required in sorted([(s,1) for s in req]+[(s,0) for s in best],key=lambda z:(-z[1],z[0].day,{'AM':0,'FULL':1,'PM':2}.get(z[0].block,9),z[0].idx)):
    e=elig(sl)
    if not e:
    ...
    desired.append((sl.idx,b,required)); load[b]+=1; same[(sl.day,sl.block,b)]+=1; pair[(b,covered)]+=1
